...
pub mod exercise;
pub mod locked_u_int;
pub mod timer;
pub mod workout_list;

#[derive(Serialize, Deserialize, Debug)]
pub struct WorkoutPlus<'a> {
  pub title: &'a str,
  pub link: &'a str,
  pub day: DayOfWeek,
  pub warmup_length: u64,
  pub workout_type: WorkoutType,
  pub sets: Vec<ExerciseSet<'a>>,
}

impl<'a> WorkoutPlus<'a> {
  pub fn from_workout(workout: Workout) -> Self {
    let mut sets: Vec<ExerciseSet> = vec![];

    for s in workout.sets {
      for e in s {
        // todo: import set here.
      }
    }

    Self {
      title: workout.title.as_str(),
      link: workout.link.as_str(),
      day: workout.day,
      warmup_length: workout.warmup_length,
  ...
  pub fn new(
    ...
    WorkoutPlus {
      title,
      link: if let Some(link) = link { link } else { "" },
      day,
      warmup_length: 60 * 5,
      workout_type,
      sets,
...
impl<'a> Default for WorkoutPlus<'a> {
  fn default() -> Self {
    WorkoutPlus::new(
      "Default Workout",
      None,
